 ...
 * ships on. Zonky resolves real postgres binaries as ordinary Maven artifacts and this class spawns
 * them as a child process: a dependency, not a daemon.
 *
 * <p><b>It is copied per module rather than shared</b>, because sharing it would mean a test-jar
 * dependency between two modules that deliberately have none. {@code maintenance} and {@code service} each carry one. The databases are named per module for the same reason a container would be — so two suites
 * cannot mean the same name.
 *
